...
def build_instrumet(path_of_excell, count_instr):
    ...
    now_instrument = Instrument(
        Number_instr, Number_ZK, Poveritel, temp_mes_arr, Hum_mes_arr)
    time.sleep(0.5)
    wb.Save()
    time.sleep(0.5)
    wb.Close()
...
class Instrument:
    ...
    def __init__(self, Number_instr, Number_ZK, Poveritel, temp_mes_arr, Hum_mes_arr):
...
def build_prot(path_of_excell, Instrument, value_general):
    Excel = win32com.client.Dispatch("Excel.Application")
    wb = Excel.Workbooks.Open(path_of_excell)
    sheet = wb.ActiveSheet
...
while i < general[4]:
    ...
    tmpPath = build_prot(generalPathsample, tmp, general)
